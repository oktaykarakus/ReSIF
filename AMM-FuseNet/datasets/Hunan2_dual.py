...
class Hunan2_dual_2D(VisionDataset):
    ...
    def __init__(
        ...
        if split == 'train':
            ...
            for image in sorted(s2):
                mask = image.replace(self.s2_root, self.mask_root)
                mask = mask.replace("s2", "lc")
                dem = image.replace(self.s2_root, self.dem_root)
                dem = dem.replace("s2", "topo")
                self.files.append(dict(s2=image, dem=dem, mask=mask))
        elif split == "test":
            s2 = glob.glob(
    ...
    def __getitem__(self, index: int) -> Dict[str, Tensor]:
        ...
        # image = torch.cat(tensors=[s2, rearrange(dem[0], " h w -> () h w")], dim=0)
        sample = {"modality1": s2, "modality2":dem, "mask": mask}
    ...
    def _verify(self) -> None:
        ...
        if os.path.exists(os.path.join(self.root, self.s2_root)):
            return
